...
export interface NodeDef {
  ...
  label: string;
  stage: StageKey;
  /** Ordinal of the human approval gate, in workflow order. */
  gate?: 1 | 2 | 3;
}

export const NODE_DEFS: NodeDef[] = [
  { id: "research_memory_build", label: "Research & Memory", stage: "knowledge" },
  { id: "literature_triage", label: "Literature Triage", stage: "gate", gate: 1 },
  { id: "gap_mining", label: "Gap Mining", stage: "knowledge" },
  { id: "hypothesis_gen", label: "Hypothesis Gen", stage: "idea" },
  { id: "select_hypothesis", label: "Select Hypothesis", stage: "gate", gate: 2 },
  { id: "spec_compile", label: "Spec Compile", stage: "experiment" },
  { id: "approve_spec", label: "Approve Spec", stage: "gate", gate: 3 },
  { id: "run_modelica", label: "Run Modelica", stage: "experiment" },
  { id: "handle_run_failure", label: "Failure Decision", stage: "gate" },
  { id: "analyze_results", label: "Analyze Results", stage: "analysis" },
  { id: "review_evidence", label: "Evidence Review", stage: "analysis" },
  { id: "draft_report", label: "Draft Report", stage: "analysis" },
];

export const PHASE_DEFS = [
  { id: "discover", label: "Discover", nodes: ["research_memory_build", "literature_triage", "gap_mining"] },
  { id: "formulate", label: "Formulate", nodes: ["hypothesis_gen", "select_hypothesis"] },
  { id: "experiment", label: "Experiment", nodes: ["spec_compile", "approve_spec", "run_modelica", "handle_run_failure"] },
  { id: "analyse", label: "Analyse & report", nodes: ["analyze_results", "review_evidence", "draft_report"] },
] as const;
...
export const STATUS: Record<string, { c: string; bg: string; label: string; dot: string; anim?: boolean }> = {
  ...
  resuming: { c: "#1d4ed8", bg: "#eff6ff", label: "resuming", dot: "#2563eb", anim: true },
...
export const METRIC_DISPLAY: Record<string, { unit: string; label: string; color: string }> = {
  ...
  E_HVAC_kWh: { unit: "kWh", label: "HVAC energy", color: "#0f766e" },
  E_HVAC_raw_kWh: { unit: "kWh", label: "raw upstream HVAC integral (diagnostic)", color: "#b45309" },
  power_screen_excluded_samples: { unit: "samples", label: "excluded power impulses", color: "#b45309" },
  E_IT_kWh: { unit: "kWh", label: "IT energy", color: "#1d4ed8" },
  PUE: { unit: "", label: "power usage effectiveness", color: "#6d28d9" },
  T_room_peak_degC: { unit: "°C", label: "peak room temperature", color: "#dc2626" },
  thermal_exceedance_degree_hours: { unit: "K·h", label: "thermal exceedance", color: "#b45309" },
  free_cooling_hours: { unit: "h", label: "free cooling", color: "#0f766e" },
  partial_mechanical_hours: { unit: "h", label: "partial mechanical", color: "#1d4ed8" },
  full_mechanical_hours: { unit: "h", label: "full mechanical", color: "#dc2626" },
  mode_switches: { unit: "", label: "mode switches", color: "#6b7280" },
};
